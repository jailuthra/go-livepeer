...
func main() {
	...
	videoToolbox := flag.Bool("videoToolbox", false, "Use macOS VideoToolbox hardware acceleration")
	...
	if accel == ffmpeg.Nvidia && len(devices) > 0 {
		data = append(data, []string{"Acceleration", "Nvidia"})
		data = append(data, []string{"GPU IDs", fmt.Sprintf("%v", strings.Join(devices, ","))})
	} else if accel == ffmpeg.VideoToolbox {
		data = append(data, []string{"Acceleration", "VideoToolbox (MacOS)"})
	}
	if *detectionFreq > 0 {
